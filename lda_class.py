...
def forward_selection_lda(X, y, cv, scoring='f1_weighted'):
    ...
    print(f"Starting Forward Selection for LDA (Criterion: {cv.n_splits}-fold CV {scoring})...")
    print("-" * 50)
    ...
    while remaining_features:
        ...
        for feature_candidate in remaining_features:
            ...
            # Initialize and evaluate the LDA model using cross-validation
            lda = LinearDiscriminantAnalysis() 
            
            # Pass the DataFrame to cross_val_score
            scores = cross_val_score(lda, X_candidate, y, cv=cv, scoring=scoring)
...
selected_feature_indices, final_score = forward_selection_lda(X_train_std_encoded, Y_train_encoded, cv=cv)
...
# Train the final LDA model on the selected features using the full training set
if selected_feature_indices:
    ...
    final_lda_model.fit(X_train_final, Y_train_encoded)
    
    # Predict class labels and probabilities on the held-out TEST set
    Y_pred = final_lda_model.predict(X_test_final)
    Y_pred_proba = final_lda_model.predict_proba(X_test_final)
    ...
    plot_confusion_matrix(conf_matrix, CLASS_LABELS, title='LDA Test Set Confusion Matrix')
    plot_multiclass_roc(Y_test_encoded, Y_pred_proba, CLASS_LABELS)
    
    print("\nFinal LDA model trained and evaluated successfully.")
else:
